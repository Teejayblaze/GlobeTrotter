                    ...
                    <div class="user-profile-wrapper wow fadeIn" data-wow-duration="2.5s" data-wow-delay=".25s">
                        ...
                        <div class="col-lg-12 pending-transactions-table-div pt-10 pb-10">
                            ...
                            <thead class="">
                                ...
                                <th class="fs-13">Customer Paid</th>
                                <th class="fs-13">Your Allocation</th>
                                <th class="fs-13">Allocation Date</th>
                            ...
                            <tbody>
                                @foreach ($asset_booking_recs as $key => $asset_booking_rec)
                                    ...
                                    <tr>
                                        ...
                                        <td class="fs-14 sm-fs-12">{{ $transaction->disbursed_bank_ref_number??'Awaiting Ref' }}</td>
                                        <td class="fs-14 sm-fs-12">&#8358;{{ number_format(floatval(str_replace(',', '', $transaction->amount)), 2, '.', ',') }}</td>
                                        <td class="fs-14 sm-fs-12">&#8358;{{ number_format(floatval(str_replace(',', '', $transaction->disbursed_amount)), 2, '.', ',') }}</td>
                                        <td class="fs-14 sm-fs-12">{{ \Carbon\Carbon::parse($asset_booking_rec->start_date)->format('jS F Y') }}</td>
                                        <td class="fs-14 sm-fs-12">
                                            @if ($transaction->subscription)
                                                <span style="color: #ec2424;font-weight: 400;">Debit (DR)</span>
                                            @elseif ($transaction->disbursed)
                                        ...
                                        <td class="fs-14 sm-fs-12">
                                            @if ($asset_booking_rec->disbursed)
                                                {{ \Carbon\Carbon::parse($transaction->date_disbursed)->format('jS F Y') }}
                                            @elseif ($transaction->subscription)
