 ...
 <section id="site-location-search">
    <div class="user-profile pt-40 pb-40 dashboard-bg-color">
        <div class="container-fluid">
            <?php $tmp = ''; $counter = 0; ?>
            @forelse ($available_asset as $key => $asset)
            <?php $key = strtolower($asset->asset_category) ."-". strtolower($asset->type); ?>
                ...
                <div class="col-lg-12">
                    <div class="login-form search-page wow fadeIn" data-wow-duration="2.5s" data-wow-delay=".75s">
                        <div class="user-profile-sidebar-top">
                            <h2 class="profile-name dashboard-search-title">{{ strtoupper($asset->asset_category . " Board") }}</h2>
                            <h3 class="profile-name dashboard-search-title">{{ $asset->type }}</h3>
                        </div>
                        <div class="user-profile-sidebar-top text-left borderless-bottom padding-margin-bottom-none"></div>
                    </div>
                    <div class="property-sort">
                        <p class="welcome-text search-welcome-text">({{ number_format($asset_category_count[$key], 0, '', ',') }}) Result found, Kindly scroll down to see more results</p>
                    </div>
                </div>
            </div>
            <div class="row mb-40">
            ...
            @if ($key === $tmp && $asset_category_count[$key] === $counter)
            </div>
            @endif
